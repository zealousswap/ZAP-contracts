...
contract Query {
    ...
    function queryBids(
        ...
        try this.revertWithQueryResult(auction, bidIds) {} catch (
            bytes memory reason
        ) {
    ...
    function queryLastBids(
        ...
        if (nextBidId == 0) {
            try this.revertWithQueryResult(auction, new uint256[](0)) {} catch (
                bytes memory reason
            ) {
                return parseRevertReason(reason);
            }
    ...
    function getAuctionParams(
        IContinuousClearingAuction auction
    ) external view returns (AuctionParams memory params) {
    ...
    function revertWithQueryResult(
        IContinuousClearingAuction auction,
        uint256[] calldata bidIds
    ) external {
        ...
        try auction.checkpoint() returns (Checkpoint memory checkpoint) {
            QueryResult memory result = _buildQueryResult(
                auction,
                checkpoint,
                bidIds
            );
    ...
    function _calculateBidAccumulation(
        ...
        Checkpoint memory startCheckpoint = auction.checkpoints(
            info.bid.startBlock
        );
        ...
        if (info.isFullyAboveClearing) {
            ...
            (info.tokensAccumulated, info.currencySpentQ96) = _calculateFill(
                info.bid,
                currentCheckpoint.cumulativeMpsPerPrice -
                    startCheckpoint.cumulativeMpsPerPrice,
            ...
            (
                info.lastFullyFilledCheckpointBlock,
                info.outbidBlock,
                info.marginalCheckpointBlock
            ) = _findExitHints(auction, info.bid, currentCheckpoint);

            if (info.lastFullyFilledCheckpointBlock > 0) {
                Checkpoint memory upperCheckpoint = auction.checkpoints(
                    info.lastFullyFilledCheckpointBlock
                );
                (
                    info.tokensAccumulated,
                    info.currencySpentQ96
                ) = _calculateFill(
                    info.bid,
                    upperCheckpoint.cumulativeMpsPerPrice -
                        startCheckpoint.cumulativeMpsPerPrice,
                    upperCheckpoint.cumulativeMps -
                        startCheckpoint.cumulativeMps,
                    mpsRemaining
                );
            }
            ...
            if (info.isOutbid && info.marginalCheckpointBlock > 0) {
                Checkpoint memory marginalCheckpoint = auction.checkpoints(
                    info.marginalCheckpointBlock
                );
                (
                    uint256 partialTokens,
    ...
    function _calculatePartialFill(
        ...
        uint256 currencyRaisedAtClearingX7 = ValueX7.unwrap(
            currentCheckpoint.currencyRaisedAtClearingPriceQ96_X7
        );
        ...
        currencySpentQ96 = bid.amountQ96.fullMulDivUp(
            currencyRaisedAtClearingX7,
            denominator
        );

        // tokensFilled = bid.amountQ96 * currencyRaisedAtClearingX7 / denominator / maxPrice
        tokensFilled =
            bid.amountQ96.fullMulDiv(currencyRaisedAtClearingX7, denominator) /
            bid.maxPrice;
    }
    ...
    function _findExitHints(
        ...
        returns (
            uint64 lastFullyFilledCheckpointBlock,
            uint64 outbidBlock,
            uint64 marginalCheckpointBlock
        )
    {
        ...
        if (currentCheckpoint.clearingPrice > bidMaxPrice) {
            ...
            while (checkpointBlock > 0 && checkpointBlock >= bidStartBlock) {
                Checkpoint memory checkpoint = auction.checkpoints(
                    checkpointBlock
                );
                ...
                if (checkpoint.clearingPrice < bidMaxPrice) {
                    ...
                    outbidBlock = prevCheckpointAboveClearing > 0
                        ? prevCheckpointAboveClearing
                        : uint64(block.number);
                    marginalCheckpointBlock = foundMarginalBlock;
                    return (
                        lastFullyFilledCheckpointBlock,
                        outbidBlock,
                        marginalCheckpointBlock
                    );
    ...
    function parseRevertReason(
        bytes memory reason
    ) internal pure returns (QueryResult memory) {
